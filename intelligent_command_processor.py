...
class IntentRecognizer:
    ...
    INTENT_PATTERNS = {
        "send_dm": [
            r"^(?:open|start)\s+(?:a\s+)?dm\s+(?:with\s+)?(.+?)(?:\s+(?:and|say|message)\s+(.+))?$",
...
class ArgumentExtractor:
    ...
    def __init__(self, bot: discord.Client):
        self.bot = bot
        self.cache = GuildIndexCache(ttl_seconds=120)
    ...
    async def extract_targets(
        ...
        cleaned_text = re.sub(r"\s+and\s+", ",", remaining_text, flags=re.IGNORECASE)
        tokens = [t.strip("\"', ") for t in re.split(r"[,\s]+", cleaned_text) if t.strip("\"', ")]
        ...
        for token in tokens:
            ...
            # Check pronouns based on context
            if token.lower() in ("him", "her", "them", "he", "she"):
            ...
            best = pick_best(candidates, min_score=0.78, gap=0.05)
            if best:
                if best not in resolved_ids:
                    resolved_ids.append(int(best))
                    label = next((c.label for c in candidates if c.entity_id == best), str(best))
                    resolved_names.append(label)
                continue

            if candidates:
                labels = ", ".join(c.label for c in candidates)
                unresolved.append(f"{token} ({labels})")
            else:
...
class IntelligentCommandProcessor:
    ...
    async def _handle_send_dm(self, intent, user, guild, channel, executor) -> bool:
        ...
        target_text = matches[0]["value"] if matches else intent.get("raw_input", "")
        targets, target_names, unresolved = await self.extractor.extract_targets(target_text, guild, context)

        if unresolved:
            await channel.send(f"?,1?,? Couldn't find or resolve: {', '.join(unresolved)}")
            if not targets:
                return False

        message = None
        if len(matches) >= 2 and matches[1]["value"]:
            message = matches[1]["value"]
        if not message:
            display = getattr(user, "display_name", None) or getattr(user, "name", "someone")
            message = f"{display} asked me to open a DM with you."

        try:
            ok = await executor.send_dm(targets, message)
            if ok:
                ...
                return True
            await channel.send("??O Failed to send DM.")
            return False
        except Exception as e:
            await channel.send(f"??O Failed to send DM: {str(e)}")
            return False
