...
class _ManydAICommandExecutor:
    ...
    def _parse_channel(self, channel_ref: str) -> Optional[int]:
        ...
        # Cross-guild search
        candidates = rank_channels_global(
            self.mandy.bot,
            name,
            prefer_guild_id=self.guild.id if self.guild else None,
            cache=self.mandy._resolver_cache,
            limit=3,
        )
        picked = pick_best(candidates, min_score=0.8, gap=0.05)
        if picked:
            return int(picked)
        return None
...
class MandyAI(commands.Cog):
    ...
    def _auto_tune(self, wait_seconds: float, source: str) -> None:
        ...
        """
        ai = self._cfg()
        cooldown = int(ai.get("cooldown_seconds", 5))
        updated = False

        if wait_seconds >= 30 and cooldown < 25:
            ai["cooldown_seconds"] = min(25, cooldown + 2)
            updated = True
        elif wait_seconds <= 5 and cooldown > 5:
            ai["cooldown_seconds"] = max(3, cooldown - 1)
            updated = True

        if source == "gemini":
            router = ai.get("router_model") or ai.get("default_model") or ""
            if "pro" in router and wait_seconds >= 30:
                ai["router_model"] = "gemini-2.5-flash-lite"
                updated = True

        if updated and self.store:
            try:
                asyncio.create_task(self.store.mark_dirty())
            except Exception:
                pass
    ...
    async def _resolve_user(
        ...
        # Prefer the current guild first
        if guild:
        ...
        # Global search across all guilds
        global_candidates = rank_members_global(
            self.bot,
            token,
        ...
        picked = pick_best(global_candidates, min_score=0.78, gap=0.05)
        if picked:
            return int(picked), []
        if global_candidates:
            labeled = [(cand.entity_id, cand.label) for cand in global_candidates[:5]]
            return None, labeled
    ...
    def _parse_dm_request(self, text: str) -> Optional[Tuple[List[str], str]]:
        ...
        if match:
            targets = self._split_targets(match.group(2).strip())
            return targets, match.group(3).strip()
        match = re.match(r"^(dm|message|privately\s+message)\s+(.+)$", text, re.IGNORECASE)
        if match:
            targets = self._split_targets(match.group(2).strip())
            return targets, ""
        return None

    def _parse_open_dm_request(self, text: str) -> Optional[List[str]]:
        match = re.match(r"^(open|start)\s+dm\s+with\s+(.+)$", text, re.IGNORECASE)
        if match:
            return self._split_targets(match.group(2).strip())
        match = re.match(r"^(open|start)\s+dm\s+(.+)$", text, re.IGNORECASE)
        if match:
            return self._split_targets(match.group(2).strip())
        return None
    ...
    async def _handle_fast_path(
        ...
        open_dm_targets = None if dm_match else self._parse_open_dm_request(text)
        if dm_match or open_dm_targets:
            targets, dm_text = dm_match if dm_match else (open_dm_targets or [], "")
            if not targets:
                await self._send_chunks(channel, "Please name at least one target.")
                return True
            dm_text = dm_text or f"{getattr(user, 'display_name', user.name)} asked me to open a DM with you."
    ...
    async def _execute_actions(
        ...
        if not self.registry:
            self.registry = CapabilityRegistry(self.tools)
        GLOBAL_USER_TOOLS = {"send_dm", "show_stats"}
        GLOBAL_MEMBERSHIP_EXEMPT = GLOBAL_USER_TOOLS | {
            "add_watcher",
            "remove_watcher",
            "list_watchers",
            "list_mirror_rules",
        ...
        for action in actions:
            ...
            if not ok:
                ...
                continue
            try:
                membership_required = tool not in GLOBAL_MEMBERSHIP_EXEMPT
                if guild and membership_required and "target_user_id" in args:
                    member = guild.get_member(int(args["target_user_id"]))
                    if not member:
                        results.append(f"{tool}: ERROR user not found in this guild")
                        continue
                if guild and membership_required and "user_id" in args:
                ...
                if dynamic:
                    ...
                    if not func:
                        if tool == "list_capabilities":
                            summary = await self._list_capabilities_summary()
                            results.append(f"{tool}: OK {summary}")
                            await self._log(f"[Mandy ACTION] user={user_id} tool={tool} args={self._summarize_args(args)} result={summary}")
    ...
    async def _list_capabilities_summary(self) -> str:
        registry = getattr(self.bot, "mandy_registry", None) or CapabilityRegistry(self.tools or self)
        ai = self._cfg()
        installed = list(ai.get("installed_extensions", []) or [])
        loaded = sorted(self.bot.extensions.keys())
        for mod in loaded:
        ...
        dynamic = []
        if registry:
            tool_registry = getattr(registry, "_tool_registry", None)
            if tool_registry:
