...
def rank_members_global(
    bot: discord.Client,
    query: str,
    prefer_guild_id: Optional[int] = None,
    cache: Optional[GuildIndexCache] = None,
    limit: int = 8,
) -> List[ResolutionCandidate]:
    """
    Search across all guilds the bot is in for a member match.

    Returns candidates sorted by score with guild hints attached.
    """
    if not bot or not query:
        return []
    all_candidates: Dict[int, ResolutionCandidate] = {}
    for guild in bot.guilds:
        index = cache.get(guild) if cache else None
        candidates = rank_members(guild, query, index=index, limit=limit)
        for cand in candidates:
            score = cand.score
            if prefer_guild_id and int(guild.id) == int(prefer_guild_id):
                score = min(0.99, score + 0.05)
            prev = all_candidates.get(cand.entity_id)
            label = f"{cand.label} @{guild.name}"
            merged = ResolutionCandidate(
            ...
            if not prev or merged.score > prev.score:
                all_candidates[cand.entity_id] = merged
    merged_list = list(all_candidates.values())
    merged_list.sort(key=lambda c: (-c.score, c.label.lower()))
...
def rank_channels_global(
    ...
    """
    Search channels across all guilds. Returns candidates with guild hints.
    """
    if not bot or not query:
        return []
    all_candidates: Dict[int, ResolutionCandidate] = {}
    for guild in bot.guilds:
        index = cache.get(guild) if cache else None
        candidates = rank_channels(guild, query, index=index, limit=limit)
        for cand in candidates:
            score = cand.score
            if prefer_guild_id and int(guild.id) == int(prefer_guild_id):
                score = min(0.99, score + 0.05)
